...
import { getMeta, hasMeta, removeMeta, setMeta } from '@domina/methods';
...
const createMetaProxy = (prefix = '') => new Proxy(Object.create(null), {
  ...
  get (target, prop) {
    if (isSymbol(prop))                            return prop === Symbol.toPrimitive ? () => read(prefix) ?? '' : undefined;
    if (prefix === '' && prop in ROOT_API)         return ROOT_API[prop];
    if (prop === 'toString' || prop === 'valueOf') return () => read(prefix) ?? ''; // damit sich ein Knoten wie sein Wert verhält
    if (prop === 'toJSON')                         return () => read(prefix);
    if (prop === 'then' || prop === 'catch')       return undefined; // sonst hält await den Proxy für ein Thenable
