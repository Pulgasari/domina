...
export const isSheet  = v => typeof CSSStyleSheet !== 'undefined' && v instanceof CSSStyleSheet;
export const isCssUrl = v => isString(v) && (/^(https?:|blob:|data:|\.{0,2}\/)/.test(v) || /\.css($|[?#])/.test(v));
export const layered = (css, layer) => layer ? `@layer ${layer} {${css} }` : String(css);
...
export const adoptStylesheet = (source, { target = document, scope = null, layer = null, key, replace = false, media } = {}) => {
